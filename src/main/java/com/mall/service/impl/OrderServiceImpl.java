...
/**
 * Created by chuck Date:2017/7/30
 */
...
public class OrderServiceImpl implements IOrderService {
    ...
    public ServerResponse createOrder(Integer userId, Integer shippginId) {
        ...
        // 生成订单
        Order order = this.assembleOrder(userId, shippginId, payment);
        if (order == null) {
    ...
    private ShippingVo assembleShippingVo(Shipping shipping) {
        ...
        shippingVo.setReceiverAddress(shipping.getReceiverName());
    ...
    private ServerResponse getCartOrderItem(Integer userId, List<Cart> cartList) {
        ...
        for (Cart cartItem : cartList) {
            ...
            orderItem.setTotalPrice(BigDecimalUtil.mul(product.getCategoryId().doubleValue(), cartItem.getQuantity()));
    ...
    public ServerResponse aliCallback(Map<String,String> params) {
        ...
        if (Const.AliPayCallBack.TRAD_STATUS_TRADE_SUCCESS.equals(tradeStatus)) {
    ...
    public void closeOrder(int hour) {
        ...
        for (Order order : orderList) {
            ...
            for (OrderItem orderItem : orderItemList) {
                // 一定要用主键where 条件, 防止锁表,同时必须支持MySql的InnoDB
            ...
            log.info("关闭的名单OrderNo:{}", order.getOrderNo());
        }
    }
